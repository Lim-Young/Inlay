...
控制面板是单个自包含 HTML，含三个面板——**总览 / ADR / Context**——以及全局 **自身 / 总览** 视角。ADR 面板会把被取代的 ADR 折叠收纳在取代它的决策之下；Context 面板渲染公共术语库与各人暂存。

> **隐私须知（分享前必读）：** 生成的文件会内联**全部**团队数据（含每个用户的 Context 暂存）。「自身视角」是**默认筛选，不是访问边界**——拿到文件的人都能看到全部内容。它的定位是**纯本地、各人各自生成**；请勿转发该文件，每个人在自己的 checkout 上跑 `inlay dashboard` 即可。
